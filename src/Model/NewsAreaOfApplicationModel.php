...
class NewsAreaOfApplicationModel extends \Contao\Model
{
    /**
     * Table name.
     *
     * @var string
     */
    protected static $strTable = 'tl_news_areaOfApplication';
    ...
    {
        $cssClasses = [
            'news_areaOfApplication_'.$this->id,
            'areaOfApplication_'.$this->id,
        ];
    ...
    {
        if (0 === \count($archives) || false === ($relation = Relations::getRelation('tl_news', 'areasOfApplication'))) {
            return null;
        }
...
WHERE {$relation['reference_field']} IN (SELECT id FROM tl_news WHERE pid IN (".\implode(',', \array_map('intval', $archives)).')';
     ...
     *
     * @return NewsAreaOfApplicationModel|null
     */
...
//            }
    ...
    {
        if (0 === \count($ids = Model::getRelatedValues('tl_news', 'areasOfApplication', $newsId))) {
            return null;
        }
    ...
    public static function findPublishedLatestNews( $companyID, $archiveIDs )
    {
        ...
        $objAllNews = \Database::getInstance()->prepare('SELECT * FROM tl_news_areasOfApplication WHERE areaOfApplication_id=?')->execute($companyID);
    ...
    public static function getUsage(array $archives = [], $company = null, $includeSubcompanies = false, array $cumulativeCompanies = [])
    {
        ...
        if (null !== $company && $includeSubcompanies) {
            $company = static::getAllSubAreasOfApplicationIds($company);
        }

        $ids = Model::getReferenceValues($t, 'areasOfApplication', $company);
        ...
        if (count($cumulativeCompanies) > 0) {
            ...
            foreach ($cumulativeCompanies as $cumulativeCompany) {
                $tmp = Model::getReferenceValues($t, 'areasOfApplication', $cumulativeCompany);

                // Include the subcompanies
                if ($includeSubcompanies) {
                    $tmp = static::getAllSubAreasOfApplicationIds($tmp);
                }
     ...
     * @return array
     */
    public static function getAllSubAreasOfApplicationIds($company)
    {
