...
class NewsUsagePickerWidget extends Widget
{
    /**
     * Submit user input.
     *
     ...
     * @var string
     */
    protected $strTable = 'tl_news_usage';
    ...
    public function generate()
    {
        ...
        // Can be an array
        if (!empty($this->varValue) && null !== ($entries = NewsUsageModel::findMultipleByIds((array) $this->varValue)))
        {
            /** @var NewsUsageModel $entry */
  ...
  <div class="selector_container">'.(($this->sorting && count($values) > 1) ? '
    ...
    <ul id="sort_'.$this->strId.'" class="'.($this->sorting ? 'sortable' : '').'">';
        ...
        if (!$pickerBuilder->supportsContext('newsUsage'))
        {
            $return .= '
	<p><button class="tl_submit" disabled>'.$GLOBALS['TL_LANG']['MSC']['changeSelection'].'</button></p>';
        ...
        else
        {
            ...
            $return .= '
	<p><a href="'.ampersand($pickerBuilder->getUrl('newsUsage', $extras)).'" class="tl_submit" id="pt_'.$this->strName.'">'.$GLOBALS['TL_LANG']['MSC']['changeSelection'].'</a></p>
	<script>
	  $("pt_'.$this->strName.'").addEvent("click", function(e) {
		...
		Backend.openModalSelector({
		  ...
		  "callback": function(table, value) {
			...
			}).post({"action":"reloadNewsUsageWidget", "name":"'.$this->strId.'", "value":value.join("\t"), "REQUEST_TOKEN":"'.REQUEST_TOKEN.'"});
		  }
		});
	  });
	</script>'.($this->sorting ? '
